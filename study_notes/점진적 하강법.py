# -*- coding: utf-8 -*-
"""
Created on Mon Apr 12 20:45:44 2021
...
def prediction(theta_0, theta_1, x):
    function = theta_0 + x * theta_1
    return function
    """주어진 학습 데이터 벡터 x에 대해서 모든 예측 값을 벡터로 리턴하는 함수"""


def prediction_difference(theta_0, theta_1, x, y):
    vector = prediction(theta_0, theta_1, x)
    difference = vector - y
    return difference
...
def gradient_descent(theta_0, theta_1, x, y, iterations, alpha):
    ...
    for i in range(iterations):  # 정해진 번만큼 경사 하강을 한다
        ...
        if i % 10 == 0:
            ...
            plt.plot(house_size, prediction(theta_0, theta_1, x),
                     color='red')
            plt.show()
...
house_size = np.array([
    0.9, 1.4, 2, 2.1, 2.6, 3.3, 3.35, 3.9, 4.4, 4.7, 5.2, 5.75, 6.7, 6.9])

# 목표 변수(집 가격) 초기화 (모든 집 값 데이터를 1/10 크기로 줄임)
house_price = np.array([
    0.3, 0.75, 0.45, 1.1, 1.45, 0.9, 1.8, 0.9, 1.5, 2.2, 1.75, 2.3, 2.49, 2.6])
...
# 학습률 0.1로 200번 경사 하강
theta_0, theta_1, cost_list = gradient_descent(
    theta_0, theta_1, house_size, house_price, 300, 0.1)
